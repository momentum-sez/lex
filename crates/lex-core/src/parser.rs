...
impl<'a> Parser<'a> {
    ...
    /// body) and encodes the rule body inline as a lambda. It is the shape
    /// used by jurisdictional rule files that encode a rule body directly
    /// rather than naming a declarative rule.
    ///
            ...
            Token::Ident(ref s) => {
                ...
                match name.as_str() {
                    ...
                    "sanctions_query" | "statutory_sanctions_query" => {
                        // Both markers share the same sanctions-query effect
                        // signature. Any jurisdiction-specific meaning of
                        // "statutory" belongs in the pack-local rule text.
                        self.advance();
