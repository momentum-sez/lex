//! # Canonical Serialization - Lex Canonical Form (LCF)
//!
...
//!
//! ## Lex Canonical Form (LCF)
//!
//! LCF is based on RFC 8785 JSON Canonicalization Scheme (JCS) with two
...
//!
//! **Digest:** `SHA-256(LCF(payload))`
//!
//! ## Security Invariant
//!
//! The inner `Vec<u8>` is private. The only way to construct `CanonicalBytes`
//! is through [`CanonicalBytes::new`], which applies the full LCF pipeline
...
/// Bytes produced exclusively by Lex Canonical Form (LCF) canonicalization.
///
/// LCF = RFC 8785 JCS + float rejection + datetime normalization to UTC seconds.
///
...
impl CanonicalBytes {
    ...
    ///
    /// Applies the full Lex type coercion pipeline before serialization:
...
/// Recursively coerce JSON values according to Lex canonicalization rules.
///
